...
export default function DrawsTable({ draws, onSelectDraw }: { draws: any[], onSelectDraw: (draw: any) => void }) {
  const getStatusPill = (status: string) => {
    switch (status) {
      ...
      case "DRAFT":
        return <span className="px-3 py-1 rounded-full border border-[#6B7280]/30 bg-[#374151] text-[#9CA3AF] font-sans font-medium text-[10px]">Draft</span>;
      case "ENDED":
        return <span className="px-3 py-1 rounded-full border border-[#4ADE80]/30 bg-[#083b18] text-[#4ADE80] font-sans font-medium text-[10px]">Completed</span>;
      case "ACTIVE":
        return <span className="px-3 py-1 rounded-full border border-[#EAB308]/30 bg-[#854D0E] text-[#FDE047] font-sans font-medium text-[10px]">In Progress</span>;
      case "CANCELLED":
        return <span className="px-3 py-1 rounded-full border border-[#EF4444]/30 bg-[#7F1D1D] text-[#F87171] font-sans font-medium text-[10px]">Cancelled</span>;
      default:
        return null;
    }
  };

  const getDrawType = (draw: any) => {
      ...
      <table className="w-full min-w-[900px] text-left border-collapse">
        <thead>
          <tr className="border-b border-[#2D3C13] bg-[#111210]">
            ...
            <th className="py-4 px-6 font-sans text-[10px] font-medium text-[#5A752A] uppercase tracking-[1px] w-[15%] text-center">DRAW TYPE</th>
            <th className="py-4 px-6 font-sans text-[10px] font-medium text-[#5A752A] uppercase tracking-[1px] w-[15%] text-center">END DATE</th>
        ...
        <tbody>
          {draws.map((draw, i) => {
            ...
            return (
              <tr key={draw.id} className={`${i !== draws.length - 1 ? 'border-b border-[#2D3C13]' : ''} hover:bg-[#1A230A] transition-colors`}>
                <td className="py-4 px-6">
                  <span className="font-sans font-medium text-[13px] text-[#E8EDD4]">{draw.title}</span>
                </td>
                <td className="py-4 px-6">
                  <div className="flex items-center gap-3">
                    <div className="w-6 h-6 rounded-full bg-[#1A230A] border border-[#43581E] flex items-center justify-center shrink-0 overflow-hidden">
                      {draw.host?.user?.avatarUrl ? (
                        <img src={draw.host.user.avatarUrl} alt="Host" className="w-full h-full object-cover" />
                      ) : (
                        <span className="font-sans font-medium text-[9px] text-[#8CB34A]">{hostInitials}</span>
                      )}
                    </div>
                ...
                <td className="py-4 px-6 text-center">
                  <span className={`font-sans font-medium text-[12px] ${getTypeStyle(drawType)}`}>{drawType}</span>
                </td>
                <td className="py-4 px-6 text-center">
