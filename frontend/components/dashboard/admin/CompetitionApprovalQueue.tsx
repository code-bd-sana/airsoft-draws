        ...
        {!isLoading && filteredRaffles.map((item: any) => {
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-wrap items-center justify-between px-5 py-3 border-b border-[#2D3C13]/60 bg-[#161810]/50 gap-3">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-[#1A230A] border border-[#43581E] flex items-center justify-center shrink-0 overflow-hidden text-[#8CB34A] font-heading font-bold text-xs">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {item.host?.isVerified && (
                      <span className="px-1.5 py-0.2 rounded bg-[#8CB34A]/20 text-[#8CB34A] text-[10px] font-semibold">
                        ✓ Verified
                      </span>
                    )}
                    <span className="text-[#5A752A] text-xs">•</span>
                ...
                <div className="flex items-center gap-2">
                  {item.host && (
                    <button
                      ...
                      className="px-2.5 py-1 rounded-[6px] bg-[#1A230A] border border-[#2D3C13] hover:border-[#8CB34A] text-[#8CB34A] text-[11px] font-sans font-medium flex items-center gap-1 transition-colors cursor-pointer"
                  ...
                  {item.category && (
                    <span className="px-2 py-0.5 rounded-[4px] bg-[#1A230A] border border-[#2D3C13] text-[#A0D056] text-[11px] font-sans">
                      {item.category}
                    </span>
                  )}
                  {item.prizeClassification && (
                    <span className="px-2 py-0.5 rounded-[4px] bg-[#2D3C13]/50 text-[#72943A] text-[10px] font-mono uppercase">
              ...
              <div className="flex flex-col md:flex-row gap-5 p-5">
                {/* Thumbnail Image */}
                <div className="w-full md:w-[150px] h-[110px] shrink-0 bg-[#1A230A] border border-[#2D3C13] rounded-[10px] flex items-center justify-center overflow-hidden relative group">
                  {item.mainImage ? (
                    <img
                      src={item.mainImage}
                      alt={item.title}
                  ...
                  {item.instantWins && item.instantWins.length > 0 && (
                    <div className="absolute bottom-1 right-1 px-1.5 py-0.5 rounded bg-black/70 border border-[#D97706]/50 text-[#F59E0B] text-[10px] font-mono font-medium">
                ...
                <div className="flex flex-col gap-2.5 flex-1 min-w-0">
                  ...
                  <h3
                    ...
                    className="font-heading font-bold text-[17px] sm:text-[18px] text-[#E8EDD4] hover:text-[#8CB34A] transition-colors cursor-pointer truncate"
                  ...
                  <p className="font-sans text-[13px] text-[#A0D056]/80 leading-relaxed line-clamp-2 max-w-[900px]">
                    {item.description || "No description provided."}
                  </p>

                  {/* PROMINENT START & END DATE AND TIME (Requested by client) */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 pt-1.5">
                    {/* Start Date & Time */}
                    <div className="flex items-center gap-2 px-3 py-2 rounded-[8px] bg-[#161810] border border-[#2D3C13]">
                      ...
                      <div className="flex flex-col min-w-0">
                        <span className="text-[10px] font-semibold text-[#8CB34A] uppercase tracking-wider">
                          Starts (Goes Live)
                        </span>
                        <span className="font-mono text-[12px] text-[#E8EDD4] font-medium truncate">
                    ...
                    {/* End Date & Time */}
                    <div className="flex items-center gap-2 px-3 py-2 rounded-[8px] bg-[#161810] border border-[#2D3C13]">
                      <div className="w-7 h-7 rounded-full bg-[#78350F]/40 border border-[#D97706]/40 flex items-center justify-center shrink-0">
                    ...
                    {/* Quick Stats Pill */}
                    <div className="flex items-center justify-between px-3 py-2 rounded-[8px] bg-[#161810] border border-[#2D3C13] sm:col-span-2 lg:col-span-1">
                      <div className="flex flex-col">
                        <span className="text-[10px] font-semibold text-[#5A752A] uppercase tracking-wider">
                          Price & Tickets
                        </span>
                        <span className="font-sans text-[12px] text-[#E8EDD4]">
                          <strong className="text-[#8CB34A]">£{ticketPrice.toFixed(2)}</strong> / ticket · <strong className="text-[#E8EDD4]">{totalTickets}</strong> total
                        </span>
                      </div>
                      {durationDays !== null && durationDays > 0 && (
                        <span className="px-2 py-0.5 rounded bg-[#1A230A] border border-[#2D3C13] text-[#72943A] text-[10px] font-mono">
              ...
              {/* Bottom Action Bar */}
              <div className="flex flex-col sm:flex-row items-center justify-between px-5 py-3.5 border-t border-[#2D3C13]/60 bg-[#161810]/40 gap-3">
                {/* View Details Button */}
                <button
                  onClick={() => setDetailsModalComp(item)}
                  className="w-full sm:w-auto h-[38px] px-4 rounded-[8px] bg-[#1A230A] hover:bg-[#2D3C13] border border-[#2D3C13] text-[#8CB34A] hover:text-[#A0D056] font-heading font-medium text-[13px] flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
                    <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                  </svg>
                  <span>View Full Details & Host Info</span>
                </button>
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto">
                  <button
                    ...
                    className="flex-1 sm:flex-none h-[38px] px-5 rounded-[8px] bg-transparent border border-[#7F1D1D] hover:bg-[#7F1D1D]/20 text-[#f76b6b] cursor-pointer font-heading font-medium text-[13px] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Reject
                  </button>
                  <button
                    ...
                    disabled={approvingId !== null}
                    className="flex-1 sm:flex-none h-[38px] px-6 rounded-[8px] bg-[#8CB34A] cursor-pointer
                     hover:bg-[#A0D056] text-[#0D0D0B] font-heading font-semibold text-[13px] transition-colors flex items-center justify-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_0_12px_rgba(140,179,74,0.25)]"
                  >
